...
export class ApiClient {
  ...
  /**
   * Sets the per-agent Codex session mode: "shared" (sidebar-visible,
   * serialized) or "isolated" (parallel). Empty mode resets to inherit the
   * daemon's global setting. Same auth/audit surface as the env endpoint.
   */
  async updateAgentCodexHomeMode(
    id: string,
    mode: "" | "shared" | "isolated",
  ): Promise<{ codex_home_mode: string }> {
    return this.fetch(`/api/agents/${id}/codex-home-mode`, {
      method: "PUT",
