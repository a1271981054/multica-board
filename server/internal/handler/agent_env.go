...
// UpdateAgentCodexHomeModeRequest is the body for PUT
// /api/agents/{id}/codex-home-mode. Mode "shared" / "isolated" sets the
// agent-level override; empty resets to inherit the daemon's global setting.
type UpdateAgentCodexHomeModeRequest struct {
	Mode string `json:"mode"`
}

// UpdateAgentCodexHomeMode is a first-class switch for the per-agent Codex
// session mode. It reuses the env-management auth + audit path but only ever
// touches the CODEX_SESSION_MODE key, so the UI never has to reveal secrets.
func (h *Handler) UpdateAgentCodexHomeMode(w http.ResponseWriter, r *http.Request) {
	agentRow, member, ok := h.authorizeAgentEnv(w, r)
	if !ok {
		return
	}

	var req UpdateAgentCodexHomeModeRequest
	if err := json.NewDecoder(r.Body).Decode(&req); err != nil {
		writeError(w, http.StatusBadRequest, "invalid request body")
		return
	}
	mode := strings.ToLower(strings.TrimSpace(req.Mode))
	if mode != "" && mode != agent.CodexSessionModeShared && mode != agent.CodexSessionModeIsolated {
		writeError(w, http.StatusBadRequest, "mode must be one of: shared, isolated")
		return
	}

	existing := unmarshalCustomEnv(agentRow)
	if mode == "" {
		delete(existing, agent.CodexSessionModeEnvKey)
	} else {
		existing[agent.CodexSessionModeEnvKey] = mode
	}
	envBytes, err := json.Marshal(existing)
	if err != nil {
		writeError(w, http.StatusInternalServerError, "failed to encode env")
		return
	}
	...
	tx, err := h.TxStarter.Begin(r.Context())
	if err != nil {
		slog.Error("agent codex-home-mode update: begin tx failed",
			append(logger.RequestAttrs(r), "error", err, "agent_id", uuidToString(agentRow.ID))...)
	...
	if err != nil {
		slog.Warn("update agent codex-home-mode failed",
			append(logger.RequestAttrs(r), "error", err, "agent_id", uuidToString(agentRow.ID))...)
		writeError(w, http.StatusInternalServerError, "failed to update agent")
		return
	}
	...
	if _, err := qtx.CreateActivity(r.Context(), db.CreateActivityParams{
		...
		slog.Error("agent codex-home-mode audit write failed; rolling back update",
			append(logger.RequestAttrs(r), "error", err, "agent_id", uuidToString(agentRow.ID))...)
		writeError(w, http.StatusInternalServerError, "audit log write failed; update rolled back")
		return
	}
	if err := tx.Commit(r.Context()); err != nil {
		slog.Error("agent codex-home-mode commit failed",
			append(logger.RequestAttrs(r), "error", err, "agent_id", uuidToString(agentRow.ID))...)
		writeError(w, http.StatusInternalServerError, "failed to update agent")
		return
	}
	...
	resp := h.agentToResponse(updated)
	if err := h.attachAgentSkills(r.Context(), &resp, updated.ID); err != nil {
		slog.Warn("load agent skills after codex-home-mode update failed",
			append(logger.RequestAttrs(r), "error", err, "agent_id", uuidToString(updated.ID))...)
		writeError(w, http.StatusInternalServerError, "failed to load agent skills")
		return
	}
	h.publish(protocol.EventAgentStatus, uuidToString(updated.WorkspaceID), "member", uuidToString(member.UserID), map[string]any{"agent": broadcastAgentResponse(resp)})

	writeJSON(w, http.StatusOK, map[string]any{"codex_home_mode": mode})
}
