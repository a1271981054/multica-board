...
interface ContentEditorRef {
  ...
  clearContent: () => void;
  focus: () => void;
  /** The editor's outer layout wrapper, for hosts that need to anchor their
   *  own floating UI to the editor. Returns null before mount / after destroy. */
  ...
  /**
   * Insert a markdown fragment at the current caret, replacing any active
   * selection. Unlike `insertMarkdownAtEnd`, this respects where the user is
   * typing, which slash-command pickers need when the caret sits mid-document.
   * `trimBefore` additionally deletes that many characters immediately before
   * a collapsed selection, so slash pickers can replace the typed `/query`
   * trigger rather than append after it.
   */
  insertMarkdownAtCursor: (markdown: string, trimBefore?: number) => boolean;
  /**
    ...
    useImperativeHandle(ref, () => ({
      ...
      insertMarkdownAtCursor: (markdown: string, trimBefore = 0) => {
        ...
        const start = editor.state.selection.empty
          ? Math.max(0, from - trimBefore)
          : from;
        editor.commands.insertContentAt({ from: start, to }, markdown, {
          contentType: "markdown",
        });
        return true;
      },
