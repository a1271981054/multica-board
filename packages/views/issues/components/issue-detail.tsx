  ...
  const reject = async () => {
    try {
      await createComment.mutateAsync({
        content: reason.trim() ? `审核打回：${reason.trim()}` : "审核打回",
        type: "comment",
      });
    } catch {
      // The status update below still applies even if the reason comment fails.
    }
    onUpdate({ status: "todo" });
    void api.rerunIssue(issueId).catch(() => {});
    setRejectOpen(false);
    setReason("");
  };
  ...
  return (
    <>
      ...
      <Dialog open={rejectOpen} onOpenChange={setRejectOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>打回任务</DialogTitle>
          </DialogHeader>
          <textarea
            className="min-h-24 w-full rounded-md border border-border bg-background p-2 text-sm"
            value={reason}
            onChange={(event) => setReason(event.target.value)}
            placeholder="填写打回原因，将写入评论并自动重新执行"
          />
          <div className="flex justify-end gap-2">
            <Button variant="outline" onClick={() => setRejectOpen(false)}>
              取消
            </Button>
            <Button onClick={() => void reject()} disabled={createComment.isPending}>
              打回并重开
            </Button>
          </div>
        </DialogContent>
        ...
        <BreadcrumbHeader
          ...
          actions={
            ...
            {issue.status === "in_review" && (
              <ReviewActions issueId={issue.id} onUpdate={handleUpdateField} />
            )}
