...
func (d *Daemon) runTask(ctx context.Context, task Task, provider string, slot int, taskLog *slog.Logger) (taskResult TaskResult, returnErr error) {
	...
	if task.Agent != nil {
		...
		codexHomeMode = strings.ToLower(strings.TrimSpace(task.Agent.CustomEnv[agent.CodexSessionModeEnvKey]))
	}
	...
	// Quick-create modal overrides win over the agent's persisted model.
	...
	// Shared Codex home is intentionally NOT serialized anymore: Codex supports
	// concurrent threads against one CODEX_HOME, and the sidebar-visible shared
	// mode must still let an agent fill its max_concurrent_tasks. We keep a
	// best-effort state backup so a task crash is recoverable.
	if env.SharedCodexHome {
		if backupErr := backupSharedCodexHome(env.CodexHome, task.ID); backupErr != nil {
			taskLog.Warn("shared codex home backup failed", "error", backupErr)
		}
	}
