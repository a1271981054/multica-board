...
export function AgentDetailInspector({
  ...
  const [codexHomeMode, setCodexHomeMode] = useState(agent.codex_home_mode ?? "");
  const [codexHomeSaving, setCodexHomeSaving] = useState(false);

  useEffect(() => {
    setName(agent.name);
    setDescription(agent.description ?? "");
    setCodexHomeMode(agent.codex_home_mode ?? "");
  ...
  const handleCodexHomeMode = useCallback(
    async (mode: "" | "shared" | "isolated") => {
      if (mode === codexHomeMode || !canEdit) return;
      setCodexHomeSaving(true);
      try {
        await api.updateAgentCodexHomeMode(agent.id, mode);
        setCodexHomeMode(mode);
        toast.success("Codex 会话模式已保存");
      } catch (e) {
    ...
    [agent.id, canEdit, codexHomeMode],
  );
    ...
    <div className="space-y-8">
      ...
      >
        <SettingsCard>
          ...
          <SettingsRow
            label="Codex 会话模式"
            description="共享：侧边栏可见但串行；隔离：并行且会话不出现在 Codex 侧边栏"
            size="none"
          >
            <CodexHomeModeField
              value={codexHomeMode}
              disabled={!canEdit || codexHomeSaving}
              onChange={handleCodexHomeMode}
            />
          </SettingsRow>
          <SettingsRow
...
function CodexHomeModeField({
  ...
  value: string;
  disabled: boolean;
  onChange: (next: "" | "shared" | "isolated") => void;
}) {
  const options: { value: "" | "shared" | "isolated"; label: string }[] = [
    { value: "", label: "跟随默认" },
    { value: "shared", label: "共享会话" },
    { value: "isolated", label: "隔离并行" },
  ];
  return (
