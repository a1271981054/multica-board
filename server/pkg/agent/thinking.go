...
// model catalog, each model's reasoning catalog, and service tiers. It
// reflects the user's live runtime config, including CC Switch's
// `model_catalog_json` when one is configured — `--bundled` would bypass
// that and show OpenAI's static catalog instead. Older versions, failed
...
// `--output json` flag. Discovery is cached, so the runtime-catalog
// round trip does not run on every poll.
//
...
// (CODEX_HOME/config.toml, falling back to ~/.codex). CC Switch writes this
