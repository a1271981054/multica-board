...
- **v0.1.18**：任务完成后如果 issue 仍停留在“进行中”，系统会自动续跑并提示智能体继续完成、将状态推进到“待审核”，不再只对从“待审核”恢复的任务生效；守护进程启动时拒绝未带版本号的 `dev` 构建，避免“未报告 CLI 版本”导致无法通过智能体创建任务。
- **v0.1.17**：在“待审核”或“已完成”（以及“阻塞”）节点上评论重新触发智能体后，任务启动时状态会自动切回“进行中”，避免智能体已经在工作但卡片仍停留在旧状态。
