...
type PrepareParams struct {
	...
	// CodexHomeMode is the agent-level Codex session mode ("shared" /
	// "isolated"). Empty inherits the daemon's global env.
...
type Environment struct {
	...
	CodexHome string
	// SharedCodexHome is true when CodexHome points at the user's shared
	// ~/.codex instead of an isolated per-task directory. In shared mode the
	// daemon serialises Codex sessions and backs up state before each run.
	SharedCodexHome bool
...
func Prepare(params PrepareParams, logger *slog.Logger) (*Environment, error) {
	...
	if params.Provider == "codex" {
		codexHome, shared := codexHomeForTask(envRoot, params.CodexHomeMode)
		if shared {
			if err := os.MkdirAll(codexHome, 0o755); err != nil {
				return nil, fmt.Errorf("execenv: prepare shared codex-home: %w", err)
			}
			env.CodexHome = codexHome
			env.SharedCodexHome = true
		} else {
...
type ReuseParams struct {
	...
	// too (MUL-4957).
	CodexCustomArgs []string
	// CodexHomeMode mirrors PrepareParams.CodexHomeMode on reuse.
	CodexHomeMode string
