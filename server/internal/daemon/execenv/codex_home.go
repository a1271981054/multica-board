...
// useSharedCodexHome reports whether the daemon should point Codex tasks at
// the user's shared ~/.codex instead of an isolated per-task home. A
// non-empty per-agent mode wins; otherwise the daemon-wide env decides.
func useSharedCodexHome(mode string) bool {
	...
	shared := strings.EqualFold(strings.TrimSpace(os.Getenv("MULTICA_CODEX_SHARED_HOME")), "true")
	isolated := strings.EqualFold(strings.TrimSpace(os.Getenv("MULTICA_CODEX_ISOLATED_HOME")), "true")
	return shared && !isolated
}

// codexHomeForTask returns the CODEX_HOME path for a Codex task and whether
// that path is the shared user home.
func codexHomeForTask(envRoot, mode string) (string, bool) {
	if useSharedCodexHome(mode) {
		return resolveSharedCodexHome(), true
	}
	return filepath.Join(envRoot, codexHomeDirName), false
}
