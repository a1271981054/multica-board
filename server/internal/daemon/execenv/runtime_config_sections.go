...
// writeFilesystemScope emits the always-on boundary rule that keeps agents
// anchored to the task workdir instead of scanning the user's whole machine.
func writeFilesystemScope(b *strings.Builder) {
	b.WriteString("## Filesystem Scope\n\n")
	b.WriteString("Your current working directory is the project workspace for this task; run `pwd` if you need to confirm it. Read, search, and edit only inside that directory unless the task or a project resource explicitly names an external path. Never scan the user's home directory, unrelated local projects, or the whole disk, and prefer targeted paths over broad search commands.\n\n")
}
