...
-- Quick-create tasks have no issue / chat / autopilot link and are keyed by
-- origin_id (the task itself), so concurrent quick-creates are safe; only
-- legacy bare tasks (all four FKs NULL, not quick-create) stay serialized for
-- the same agent.
UPDATE agent_task_queue
          ...
          WHERE active.agent_id = atq.agent_id
            ...
            AND (
              ...
              OR (
                ...
                AND COALESCE(atq.context->>'type', '') <> 'quick_create'
