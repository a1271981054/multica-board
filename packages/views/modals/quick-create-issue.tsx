...
export function AgentCreatePanel({
  ...
  useLayoutEffect(() => {
    ...
    const anchor = editorRef.current?.getElement();
    if (!popup || !anchor) return;

    const update = () => {
      if (!popup) return;
      const width = Math.min(anchor.getBoundingClientRect().width, 640);
  ...
  const modelsQuery = useQuery(runtimeModelsOptions(selectedAgent?.runtime_id));
  ...
  const activeModelEntry = useMemo(
    () =>
      modelCatalog.find((m) => m.id === modelOverride) ??
      (modelOverride ? undefined : modelCatalog.find((m) => m.id === selectedAgent?.model)),
    [modelCatalog, modelOverride, selectedAgent?.model],
  );
    ...
    onSubmit: async (md): Promise<boolean> => {
      ...
      try {
        await api.quickCreateIssue({
          ...
          ...(thinkingOverride ? { thinking_level: thinkingOverride } : {}),
        });
  ...
  return (
    <>
        ...
        {slashOpen && slashItems.length > 0 && createPortal(
          <div
            ref={slashPopupRef}
            onMouseDown={(e) => e.preventDefault()}
            className="fixed z-[60] max-h-[min(50vh,24rem)] overflow-y-auto rounded-md border border-border/60 bg-background p-1 shadow-lg"
          >
            {slashItems.map((item) => (
              <button
        ...
        <div className="px-5 pb-2 text-caption text-muted-foreground">
          输入 / 唤起模式与技能
        </div>
          ...
          {selectedAgent && (
            <span className="flex items-center gap-1.5 rounded-md border border-border/70 bg-background/60 px-2 py-1 text-caption text-muted-foreground">
              ...
              <ModelPicker
                ...
                runtimeOnline={Boolean(selectedRuntime)}
                value={modelOverride}
                canEdit
                variant="chip"
                showLabel={false}
                onChange={(next) => {
                  ...
                  setThinkingOverride("");
                  setAgent({ model: next || undefined, thinkingLevel: undefined });
                }}
              />
            </span>
          )}
          {selectedAgent && (
            <span className="flex items-center gap-1.5 rounded-md border border-border/70 bg-background/60 px-2 py-1 text-caption text-muted-foreground">
              ...
              <ThinkingPicker
                ...
                levels={activeModelEntry?.thinking?.supported_levels ?? []}
                canEdit={(activeModelEntry?.thinking?.supported_levels.length ?? 0) > 0}
                variant="chip"
                showLabel={false}
                onChange={(next) => {
                  setThinkingOverride(next);
                  setAgent({ thinkingLevel: next || undefined });
                }}
              />
            </span>
          )}
