...
func TestGetConfigLocalAutoLoginOnlyWhenEnabled(t *testing.T) {
	t.Setenv("MULTICA_BOARD_AUTO_LOGIN", "true")
	t.Setenv("MULTICA_BOARD_AUTO_LOGIN_EMAIL", "local@multica.local")
	t.Setenv("MULTICA_DEV_VERIFICATION_CODE", "123456")
	...
	if cfg.AutoLoginEmail != "local@multica.local" || cfg.AutoLoginCode != "123456" {
		t.Fatalf("auto-login fields not emitted: %+v", cfg)
	}

	t.Setenv("MULTICA_BOARD_AUTO_LOGIN", "false")
	w = httptest.NewRecorder()
	testHandler.GetConfig(w, req)
	var cfg2 AppConfig
	if err := json.Unmarshal(w.Body.Bytes(), &cfg2); err != nil {
		t.Fatalf("decode config: %v", err)
	}
	if cfg2.AutoLoginEmail != "" || cfg2.AutoLoginCode != "" {
		t.Fatalf("auto-login fields leaked without flag: %+v", cfg2)
	}
}
