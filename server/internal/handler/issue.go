...
// up as a 500.
var validIssueStatuses = []string{"backlog", "todo", "in_progress", "in_review", "needs_input", "done", "blocked", "cancelled"}
...
func (h *Handler) QuickCreateIssue(w http.ResponseWriter, r *http.Request) {
	...
	task, err := h.TaskService.EnqueueQuickCreateTask(r.Context(), wsUUID, requesterUUID, agentUUID, squadUUID, prompt, priority, dueDate, strings.TrimSpace(req.Model), strings.TrimSpace(req.ThinkingLevel), projectUUID, parentIssueUUID, attachmentIDs)
	if err != nil {
