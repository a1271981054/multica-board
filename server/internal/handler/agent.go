...
type AgentResponse struct {
	...
	// CodexHomeMode is the per-agent Codex session mode: "shared" (visible in
	// the Codex sidebar, serialized) or "isolated" (parallel, task-local home).
	// Empty = inherit the daemon's global CODEX_HOME setting.
	CodexHomeMode     string `json:"codex_home_mode,omitempty"`
...
func (h *Handler) agentToResponse(a db.Agent) AgentResponse {
	...
	if a.CustomEnv != nil {
		...
		mode := strings.ToLower(strings.TrimSpace(customEnv[agent.CodexSessionModeEnvKey]))
		if mode == agent.CodexSessionModeShared || mode == agent.CodexSessionModeIsolated {
			codexHomeMode = mode
		}
	}
