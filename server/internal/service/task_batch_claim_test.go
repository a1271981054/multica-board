...
// claim (MUL-4257): a single call claims across all runtimes and lets one
// agent fill its max_concurrent_tasks (agent1 has two different issues), then
// reports empty once nothing is queued.
func TestClaimTasksForRuntimes_MultiRuntimeDrain(t *testing.T) {
	...
	// Call 1: agent1 has two queued tasks on different issues (capacity 5) and
	// agent2 has one => all three are claimed in one batch.
	...
	if seen[rt1] != 2 || seen[rt2] != 1 {
		t.Fatalf("call1 runtime distribution = %v, want 2 tasks on rt1 and 1 on rt2", seen)
	}
